...
`development-workflow`（简称 `dw`）是一组面向 Codex 的开发交接 skill，把产品意图逐步收敛为可执行开发任务：

```text
PRD → technical spec/plan → development prompt
```

仓库同时提供 plugin-compatible 打包、可复现验证和脱敏后的评估证据。三个 skill 职责独立，通过文档路径、批准状态和显式交接字段协作，不依赖本机安装路径或插件缓存。
...
| [工作流与文档契约](docs/workflow.md) | 三阶段职责、默认路径和批准门 |
