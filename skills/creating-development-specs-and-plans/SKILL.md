...
8. After plan approval, revalidate the PRD, spec, plan, and complete fourteen-field handoff. Freeze the canonical English snapshot, then pre-render and validate the authoritative Chinese fourteen-field view before selecting any runtime capability and before session routing. Only after that validation succeeds, if `implementation_gate` is open, select the runtime-exposed skill capability `generating-development-prompts` in the same session for session routing. If that capability is unavailable, report the capability gap without searching installation paths.
9. Every reply, including a routing reply, ends with the fixed Chinese fourteen-field status view from the handoff reference unless Chinese view validation fails. That mapping failure is the only explicit exception: report the deterministic Chinese blocker without a status view and stop the current automatic handoff. A successful routing response must end with the same fourteen-field snapshot frozen before routing.
...
- End every reply, including a clarification, ordinary blocked, capability-gap, or routing reply, with the complete fourteen-field handoff record. Chinese mapping failure is the only explicit exception and follows the fail-closed rule below.
