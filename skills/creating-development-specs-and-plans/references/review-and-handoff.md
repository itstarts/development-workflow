...
## Review Scope and Finding Contract

The approved PRD is the product-scope ceiling. A candidate spec or plan may add only the minimum technical detail needed for an approved acceptance criterion or non-goal, a confirmed repository risk, or an applicable rule; it is not authority to expand product scope. Give the approved PRD to every package, spec, and plan reviewer. When a manual prompt has no reliable PRD, use the explicitly frozen and approved scope as the same ceiling.

Classify each finding as exactly one of:

- `BLOCKING_IN_SCOPE`: the candidate violates an approved acceptance criterion or non-goal, leaves a confirmed risk unresolved, violates an applicable rule, or introduces a regression in the current candidate. This is the only classification the main agent fixes inside the current review loop.
- `SCOPE_CHANGE_REQUIRED`: resolving the concern would add product behavior, persistent identity, state, trust boundaries, shared frameworks, infrastructure, acceptance scope, or validation scope outside the approved ceiling. Do not implement it as a review fix. If the approved requirement truly cannot be satisfied without this change, explain why a simpler in-scope approach cannot work, return to requirements and user approval, and use `BLOCKED` rather than treating the candidate as directly fixable.
- `NON_BLOCKING_NOTE`: an optional improvement, preference, or residual observation that is useful to retain but is not required for the current approval.

Every finding must state its classification, precise basis and locator, current evidence and impact, minimum in-scope correction when applicable, and scope effect. A blocking basis must map to the approved PRD acceptance criteria or non-goals, a confirmed risk, an applicable rule, or a regression caused by the current candidate. A preferred architecture, generic hardening idea, speculative failure, or broader test strategy is not a blocker without that mapping. Do not fix every finding; fix only justified `BLOCKING_IN_SCOPE` findings and report the other classifications without silently expanding the candidate.

The first review checks the complete current candidate. Re-review by the same reviewer checks prior blocking findings, changed regions, and regressions caused by the correction. A new blocker is allowed only for a regression introduced by the correction or a direct overlooked violation of the approved PRD or applicable rule; state why it was not identified previously. Do not restart an unrestricted or speculative full audit on every repair cycle.

## Specification Review

Dispatch a fresh read-only spec reviewer that did not write the spec. Give it the approved PRD, user request, applicable rules, current spec, and relevant repository evidence. Do not provide an expected verdict, author defense, or requested approval.
...
On the full route, require classified findings ordered by severity, open questions, verification gaps, and one final verdict. Fix only `BLOCKING_IN_SCOPE` findings and re-review only the correction delta defined above until the spec is approved or genuinely blocked. The reviewer stays read-only; after approval of the latest file, the main agent updates independent-review metadata in the spec's current reliable schema. A reviewer verdict does not equal user approval. Ask the user only after independent spec review passes; the user explicitly approves the current written spec before creating the plan. A material spec change invalidates approval from both the independent reviewer and the user, removes their current-schema dates and reviewer metadata, then restarts both gates. On the standard route, the package-review section above owns this sequence.

## Plan Review

On the full route, dispatch a fresh read-only plan reviewer that did not write the plan. Give it the approved PRD, approved spec, applicable rules, current plan, relevant repository evidence, and available verification evidence. Require only requirements-bounded coverage, path, interface, ordering, testing, documentation, and validation checks. Include security review only when the approved requirements, confirmed risk, or applicable rule contains a real security, permission, or sensitive-data boundary. On the standard route, do not add this second reviewer; the one package reviewer already owns both files.
