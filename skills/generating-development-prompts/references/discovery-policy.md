...
For every frontmatter, parse only simple flat `key: scalar` entries. Accept an ASCII key matching the legacy key grammar or an exact Chinese key from this fixed plan set: `文档类型`, `主题`, `技术规格`, `技术规格用户批准`, `评审模式`, `计划评审状态`, `计划评审角色`, and `计划评审日期`. Classify the schema before reading status. A mixed schema, semantic duplicate, unknown Unicode key, malformed line, nested value, multiline scalar, unrecognized quoting, or unclosed frontmatter is `unknown`. Do not fall back to the header form when frontmatter exists.

An english-legacy plan or legacy header keeps the existing review-only contract: require one `review_status` to classify review, map case-insensitive `approved` to `approved`, map another explicit scalar state to `not-approved`, and preserve optional unique `reviewer` and `reviewed_at` values. For compatibility, derive `implementation_gate: open` only from approved review and `blocked` otherwise. Do not add document type, topic, spec-path, or review-mode requirements to this legacy schema.

A complete Chinese plan requires exactly one `文档类型: 实施计划`, a non-reserved kebab-case `主题`, a non-empty `技术规格`, one supported spec-approval state, and one supported plan-review state. `评审模式` accepts `技术包 | 逐级`; it may be absent only for compatibility when spec approval is already `已批准`. A `技术包` plan may preserve `技术规格用户批准: 待批准`; its plan review is still derived normally, but `implementation_gate` remains `blocked` until that copied state becomes `已批准`. A `逐级` plan with pending spec approval is unreliable. Map `计划评审状态: 已通过` to `approved` only when unique non-empty `计划评审角色` and ISO `计划评审日期` fields are present; preserve those two technical values. Map `计划评审状态: 待评审` to `not-approved` only when reviewer and date are absent. Derive `implementation_gate: open` only when both spec approval and plan review are approved; otherwise use `blocked`. A missing required field, stale lifecycle field, invalid date, or unsupported localized value is `unknown`; reviewer or date never substitutes for approval.
