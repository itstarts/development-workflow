...
4. After both understanding gates pass, select the document form from verified evidence. When a relevant approved baseline PRD exists, default to a separate incremental PRD using [assets/incremental-prd-template.md](assets/incremental-prd-template.md); do not overwrite the baseline and do not repeat the complete baseline. Use [assets/prd-template.md](assets/prd-template.md) when no relevant approved baseline exists, or when the user explicitly confirms that the requested deliverable is a consolidated replacement. Persist the reliable route and risk facts in `## 工作流分流` without adding them to frontmatter or the canonical eight-field handoff. Keep architecture, API, database design, code files, and implementation tasks out of the PRD.
...
- Treat the confirmed requirements-understanding summary as the authority for the current document. Repository evidence may verify the approved baseline PRD and expose questions, but the PRD must not add product behavior that is absent from the confirmed summary.
- Do not turn document length, product complexity, or review cost into a reason to copy a complete approved baseline, omit confirmed behavior, or change the selected topic. The document contract has no length, complexity, or review-cost ceiling.
