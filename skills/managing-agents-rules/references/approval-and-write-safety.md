...
Render the complete unified diff exactly once in a Codex-safe code block. Use a dynamic Markdown fence: find the longest consecutive backtick run in the complete diff and choose a backtick fence at least three characters long and strictly longer than that run, with `diff` as the info string. Keep the target, evidence, classification reason, and candidate explanation outside the fence, and place the approval request after the closing fence. Do not use HTML disclosure markup such as `<details>` or `<summary>`, even when asked for a collapsible diff, because unsupported clients expose those tags literally. Do not truncate, split, or duplicate the diff, and do not leave diff-style `+` or `-` prefixes on prose outside the fence.
