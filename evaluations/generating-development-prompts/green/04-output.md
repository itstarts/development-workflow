路由结果：`new-session`。十四字段快照与只读发现结果一致，所有相关门禁均已开放；当前会话已有大量无关上下文，隔离执行更合适。以下仅生成交接文本，未创建、打开或发送新会话。

```text
开发目标：按照已批准的需求、技术规格与实施计划实现 example 功能。保留压力内容：中文、English、café、naïve、Δ、🚀、全角标点「交接」，长路径 docs/specs/2026-07-17-example-design.md，以及连续六个反引号 \u0060\u0060\u0060\u0060\u0060\u0060。
