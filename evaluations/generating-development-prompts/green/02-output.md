建议路由至新会话。当前会话存在大量无关且未解决的上下文；实施计划又包含顺序任务、逐项 TDD 和多轮独立评审，隔离执行更可靠。仓库、文档与批准状态均无跨会话阻塞。以下仅生成提示词，未创建、打开或发送新会话。

```text
开发目标：依据已批准的需求、技术规格与实施计划，实现 example 功能；严格按计划顺序逐项 TDD，并进行逐项独立评审与集成后的全范围独立评审。
