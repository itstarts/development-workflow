...
理由：当前会话包含多个无关主题和大量未解决讨论；已批准计划又包含顺序 TDD 任务及最终独立评审，隔离执行能显著降低上下文干扰。仓库、文档、权限和审批均无跨会话阻塞。以下仅生成提示词，未创建、打开或发送新会话。

```text
开发目标：依据 docs/requirements/2026-07-17-example.md、docs/specs/2026-07-17-example-design.md 和 docs/plans/2026-07-17-example.md 实施 example 功能；严格按已批准计划的顺序任务逐项采用 TDD，先写并确认失败测试，再作最小实现和受影响范围验证；集成后完成一次实现独立的最新完整 diff 评审，并由同一评审者复查范围内修复直至 APPROVED。
...
规则与文档优先级
- /workspace/fixture/AGENTS.md（来源：session；优先级：0）
