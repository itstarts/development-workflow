建议去向：新会话（new-session）。

依据是当前会话已有多个无关主题、未解决讨论和明显上下文负担；获批计划包含顺序任务、逐项 TDD 及集成后的独立评审，隔离执行更合适。只读发现确认仓库、规格和计划均有效，工作树干净，未发现跨会话阻断。本回复未创建、打开或发送新会话，也未开始实施。

```text
开发目标：依据已批准的需求、技术规格与实施计划实现 example 功能，严格按计划顺序逐项执行 TDD（先写并运行失败测试），完成每项任务的受影响范围验证；全部集成后，由未参与实现的独立评审者审查最新完整 diff，并由同一评审者复审范围内修复直至批准。
