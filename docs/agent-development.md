...
| `final-reviewer` | 项目配置稳定可加载 | 一个已批准实施范围的整体完成门；与 workflow 最终角色共享完成门检查，但不负责完整五-skill 发布证据 | 规格、计划、批准范围、完整 diff、任务级评审和验证证据 | findings、开放问题、验证缺口、残留风险或 `APPROVED` | 只读；不实施、不提交、不执行安装、不改变任何外部状态 | 已批准范围一致、验证完整且该实施的完成门全部满足 | 保留 |
| `workflow-final-reviewer` | 项目配置稳定可加载 | 完整五-skill plugin、文档交接、规则治理入口和发布证据；覆盖面大于通用最终评审 | 全部 skill 项目专属证据、任务与集成评审、manifest、隔离安装和发布边界 | findings、开放问题、验证缺口、残留风险或 `APPROVED` | 只读；不安装、不 push、不发布、不改变任何外部状态 | 五-skill 合同、集成、安装与发布边界证据完整且没有阻断问题 | 保留 |
