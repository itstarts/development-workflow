...
GREEN 结果必须用 `fresh_cases` 列出 production 变化后真实重跑且纳入本次评审的 case，并包含 current RED 的 selected case。严格 freshness 不绑定固定内容哈希：干净 Git 树按 commit 祖先关系验证证据顺序；工作区可以复用已满足祖先关系的干净前序，但从第一个变化阶段起必须形成连续 dirty 后序，current RED 两份证据和全部 fresh outputs 各自保持完整，任何中间缺口、部分刷新或过期干净前序都失败；非 Git 副本标记为未验证。
...
| `final-reviewer` | 项目配置稳定可加载 | 一个已批准实施范围的整体完成门；与 workflow 最终角色共享完成门检查，但不负责完整五-skill 发布证据 | 规格、计划、批准范围、最新完整 diff、风险里程碑评审（若有）和验证证据 | findings、开放问题、验证缺口、残留风险或 `APPROVED` | 只读；不实施、不提交、不执行安装、不改变任何外部状态 | 已批准范围一致、验证完整且该实施的完成门全部满足 | 保留 |
| `workflow-final-reviewer` | 项目配置稳定可加载 | 完整五-skill plugin、文档交接、规则治理入口和发布证据；覆盖面大于通用最终评审 | 全部 skill 项目专属证据、风险里程碑评审（若有）、集成评审、manifest、隔离安装和发布边界 | findings、开放问题、验证缺口、残留风险或 `APPROVED` | 只读；不安装、不 push、不发布、不改变任何外部状态 | 五-skill 合同、集成、安装与发布边界证据完整且没有阻断问题 | 保留 |
...
任务开始时，如果全部 exposed skill 仍为 `review-approved`，先运行一次完整门以提前识别既有证据问题。最终评审前，把全部受影响 skill 放在同一条定向命令中；定向模式根据各自的 `implemented | review-approved` 自动选择 freshness validator 目标，并且仓库测试只运行一次。由一位未参与实现的 reviewer 检查最新完整 diff；批准状态写回后直接运行一次统一完整门，没有新 diff 或失败时不重复定向门。完整模式要求全部 exposed skill 已 `review-approved`。
