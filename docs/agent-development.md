...
5. 由未参与实现的只读 reviewer 依据已批准需求或冻结范围、适用规则、已确认风险检查最新 diff、证据、打包和交接契约；每项 finding 必须分类并给出具体依据与最小范围内修正。
6. 只修复 `BLOCKING_IN_SCOPE`，重新验证后由同一 reviewer 复审原阻断项、变更区域和修复回归；扩围返回需求与用户批准，非阻断建议不进入修复循环。
...
|---|---|---|---|---|---|---|---|
| `skill-reviewer` | 项目配置稳定可加载 | 单个 skill 的行为、TDD 证据、打包与跨 skill 契约；与最终角色共享 diff/验证检查，但范围止于一个 skill | 该 skill 的适用规则、已批准需求或冻结合同、最新 diff、RED/GREEN、安装边界和验证结果 | 三类 findings、依据、最小范围内修正、验证缺口、残留风险或 `APPROVED` | 只读；不提交、不安装、不改变任何外部状态；不把偏好方案变成阻断项 | 当前 skill 的项目专属证据范围完整且没有未解决的 `BLOCKING_IN_SCOPE` 或必须先扩围的阻塞 | 保留 |
| `final-reviewer` | 项目配置稳定可加载 | 一个已批准实施范围的整体完成门；与 workflow 最终角色共享完成门检查，但不负责完整六-skill 发布证据 | 已批准需求或冻结范围、规格、计划、最新完整 diff、风险里程碑评审（若有）和验证证据 | 三类 findings、依据、最小范围内修正、开放问题、验证缺口、残留风险或 `APPROVED` | 只读；不实施、不提交、不执行安装、不改变任何外部状态；复审只看原阻断项、变更区域和修复回归 | 已批准范围一致、验证完整、没有未解决的 `BLOCKING_IN_SCOPE` 或必须先扩围的阻塞，且完成门全部满足 | 保留 |
| `workflow-final-reviewer` | 项目配置稳定可加载 | 完整六-skill plugin、总路由、文档交接、规则治理入口和发布证据；覆盖面大于通用最终评审 | 全部 skill 的已批准需求或冻结合同、项目专属证据、风险里程碑评审（若有）、集成评审、manifest、隔离安装和发布边界 | 三类 findings、依据、最小范围内修正、开放问题、验证缺口、残留风险或 `APPROVED` | 只读；不安装、不 push、不发布、不改变任何外部状态；复审只看原阻断项、变更区域和修复回归 | 六-skill 合同、集成、安装与发布边界证据完整，且没有未解决的 `BLOCKING_IN_SCOPE` 或必须先扩围的阻塞 | 保留 |

角色必须保持只读、职责单一，不固定模型或 reasoning effort，不声称拥有运行时未暴露的权限。三类 finding 的共同含义为：`BLOCKING_IN_SCOPE` 直接映射批准需求、适用规则、已确认风险或当前 diff 回归；`SCOPE_CHANGE_REQUIRED` 返回需求与用户批准；`NON_BLOCKING_NOTE` 不阻断当前候选。新增或修改角色时需同步说明其触发范围、输入证据、批准条件和外部状态边界。
