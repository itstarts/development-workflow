...
当前稳定版本为 `v0.1.3`，catalog 和 plugin entry 均固定到同一个已验证 tag。`codex plugin marketplace add ... --ref <tag>` 只固定 marketplace catalog 的快照，不能覆盖 entry 自己的 plugin Git ref。发布后续不可变 tag 时，维护者必须先把 `.agents/plugins/marketplace.json` 中的 `source.ref` 更新为与 manifest 版本一致的 tag，完成候选验证后再创建该 tag；使用者随后以同一个 `--ref <tag>` 添加 catalog。
